module TurtleSearch
    export Turtles, next!, Action, findfirstseed!, bisearch, exploremap
    """
...
```
    ...
    """
        `exploremap(poses, m)` returns the path starting from the given seed, and try to find the seed of the next path.
    # Arguments
    - `poses`: the starting seed of the current path
    - `m`: the map of matrix
    # Returns
    - `xs`: the x-axis coordinates of the path
    - `ys`: the y-axis coordinates of the path
    - `poses`: the starting seed of the next path. If the next path does not exist, return `nothing`.
    # Examples
    ```jldoctest
    using .TurtleSearch
    m = [sin(y-x- pi/51) for x in range(0, stop=pi, length=50), y in range(0, stop=pi, length=50)]
    poses = CartesianIndex(1,1)
    xs, ys, poses = exploremap(poses, m)
    ```
    """
    function exploremap(poses, m::AbstractMatrix)
        t = Turtles(max(poses...))
        inds = bisearch(t, m)
        xs = [m.xaxis(inds[1])]
        ys = [m.yaxis(inds[2])]
        while !isnothing(next!(t, m))
            inds = bisearch(t, m)
